...
class ExplainNode(nodeData: JsValue, parent: ExplainNode) {
  ...
  private def matchDescriptionToString(string: String): Boolean = string.r.findAllIn(description).nonEmpty

  val nodeType: String = {
    if (matchDescriptionToString("max plus [0-9]*\\.?[0-9]* times others of:")) {
      "max"
    } else if (matchDescriptionToString("sum of:")) {
      "sum"
    } else if (matchDescriptionToString("result of:")) {
      "result"
    } else if (matchDescriptionToString("product of:")) {
      "product"
    } else {
      "leaf"
    }
  ...
  val absoluteImpactPercentage: Double = {
    if (parent == null) 100.0
    else parent.nodeType match {
      case "sum" => getPercentageForSumParent
      case "max" => getPercentageForMaxParent
      case "product" => getPercentageForProductParent
      case _ => 0.0
    }
  }
  ...
  private def getPercentageForSumParent: Double = {
    var scoreRelativeToParent = (100.0 / parent.score) * score
